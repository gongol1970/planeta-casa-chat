...
# REPORTES_ARCA_COMPACTO_FACTURA_V6 - factura layout final + ARCA compacto + canal MANUAL + usuario ML
APP_VERSION = "0.1.77-reportes-arca-compacto-factura-v6"
...
def build_recent_ml_items(rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    # Orden visual correcto: fecha/hora real de marketplace descendente, no fecha de inserción ERP.
...
def recent_customers_endpoint(
    ...
    rows = (
        sb.table("erp_customers")
        .select("id,source_origin,source_label,name,fiscal_name,doc_type,doc_number,iva_condition,email,phone,address,city,province,zip,last_order_channel,last_order_id,last_order_date,updated_at")
        .order("updated_at", desc=True)
        .limit(limit)
        .execute()
        .data
    ...
    return {"ok": True, "version": APP_VERSION, "limit": limit, "total": len(rows), "items": rows}
...
def arca_normalize_invoice_payload_for_db(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if p.get("importe_total") is None and p.get("imp_total") is not None:
        p["importe_total"] = p.get("imp_total")

    p["imp_neto"] = p.get("importe_neto") or 0
    p["imp_iva"] = p.get("importe_iva") or 0
    p["imp_total"] = p.get("importe_total") or 0
    ...
    for field in numeric_fields:
        ...
        if p.get(field) is None:
            if field in ["iva_contenido", "imp_neto", "imp_iva", "imp_total", "imp_tot_conc", "imp_op_ex", "imp_trib"]:
                p[field] = 0
            continue
        try:
...
def arca_order_report_source_id(item: Dict[str, Any], channel: str) -> str:
    ...
    if ch == "ML":
        ...
        if ids:
            return str(ids[0])
    return str(item.get("tn_order_number") or item.get("external_order_id") or item.get("id") or "")
    ...
    for ch in channels:
        ...
        for item in items:
            ...
            pending.append({
                ...
                "source_order_api_id": (item.get("child_order_ids") or [item.get("external_order_id") or item.get("id")])[0] if ch == "ML" else item.get("external_order_id"),
