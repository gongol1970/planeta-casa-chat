...
APP_VERSION = "0.1.40-arca-ml-pack-invoice-fix"
...
def arca_gross_line_total(line: Dict[str, Any]) -> float:
    qty = arca_float(line.get("quantity") or line.get("qty") or 1, 1)
    unit = arca_float(line.get("unit_price") or line.get("price") or 0, 0)
...
def arca_build_lines_from_order(row: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    # Fallback de pruebas: si tenemos ítems pero todos vinieron sin precio, repartimos el total.
    # Sirve para validar layout sin CAE. En emisión real deberían venir precios de la venta.
    if lines and order_total > 0 and line_total <= 0.50:
        total_qty = sum(max(arca_float(x.get("quantity"), 1), 0) for x in lines) or 1
        unit_fallback = round(order_total / total_qty, 2)
        for x in lines:
            x["unit_price_gross"] = unit_fallback
            x["source"] = f"{x.get('source') or 'line'}_price_fallback"
    ...
    # En ML no inventamos envío: si Mercado Libre no lo entrega como costo real, no se factura por fallback.
    ...
    if not lines:
        # Último fallback: una línea resumen para no emitir/imprimir un comprobante vacío.
        lines.append({
            "sku": "venta",
            "description": f"Venta {row.get('channel')} {arca_source_number(row)}",
            "quantity": 1,
            "unit_price_gross": order_total,
            "discount_pct": 0,
            "iva_pct": ARCA_IVA_ALICUOTA,
...
def arca_lines_from_invoice_or_raw(inv: Dict[str, Any], letter: str) -> List[Dict[str, Any]]:
    ...
    total = arca_invoice_total(inv)
    if total > 0:
        desc = f"Venta {inv.get('source_channel') or ''} {inv.get('source_order_number') or inv.get('source_order_id') or ''}".strip()
        calc = arca_calculate_invoice([{
            "sku": "venta",
            "description": desc or "Venta",
            "quantity": 1,
            "unit_price_gross": total,
            "discount_pct": 0,
            "iva_pct": ARCA_IVA_ALICUOTA,
            "source": "fallback_invoice_total",
        }], letter)
        return calc.get("lines") or []
...
def arca_invoice_html(inv: Dict[str, Any]) -> str:
    ...
    title = inv.get("cbte_tipo_label") or f"Factura {letter}"
    status = inv.get("status") or "draft"
    cae = inv.get("cae") or ""
    cae_vto = inv.get("cae_vto") or ""
    cbte_nro = inv.get("cbte_nro")
    nro = f"{int(inv.get('pto_vta') or ARCA_PTO_VTA):04d}-{int(cbte_nro or 0):08d}" if cbte_nro else f"{int(inv.get('pto_vta') or ARCA_PTO_VTA):04d}-BORRADOR"
    is_a = letter == "A"
    ...
    for line in lines[:12]:
        iva_cell = '<td class="c">21%</td>' if is_a else ''
        rows.append(f"""
        <tr>
          <td class="c">{html_lib.escape(str(line.get('quantity') or ''))}</td>
          <td>{html_lib.escape(str(line.get('sku') or ''))}</td>
          <td>{html_lib.escape(str(line.get('description') or ''))}</td>
          <td class="r">{arca_money(line.get('unit_price_display'))}</td>
          <td class="r">{arca_float(line.get('discount_pct')):.0f}%</td>
          <td class="r">{arca_money(line.get('subtotal_display'))}</td>
          {iva_cell}
        </tr>
        """)
    empty_rows = max(0, 12 - len(rows))
    col_count = 7 if is_a else 6
    for _ in range(empty_rows):
        rows.append(f"<tr class='empty'><td colspan='{col_count}'>&nbsp;</td></tr>")

    headers_extra = "<th style='width:12mm'>IVA</th>" if is_a else ""
    if is_a:
        iva_block = f"""
          <div class="totrow"><span>Subtotal bruto</span><b>{arca_money(totals.get('importe_neto'))}</b></div>
          <div class="totrow"><span>Subtotal neto</span><b>{arca_money(totals.get('importe_neto'))}</b></div>
          <div class="totrow"><span>Importe IVA 21%</span><b>{arca_money(totals.get('importe_iva'))}</b></div>
        """
    else:
        iva_block = f"""
          <div class="fiscalBox">
            <b>Régimen de Transparencia Fiscal al Consumidor (Ley 27.743)</b><br>
            IVA contenido - 21%: {arca_money(totals.get('iva_contenido'))}
          </div>
          <div class="totrow"><span>Subtotal bruto</span><b>{arca_money(totals.get('importe_total'))}</b></div>
          <div class="totrow"><span>Subtotal neto</span><b>{arca_money(totals.get('importe_total'))}</b></div>
        """

    warning = "" if cae else "<div class='draft'>BORRADOR ERP - NO FISCAL / SIN CAE</div>"
...
<head>
<meta charset="utf-8">
<title>{html_lib.escape(title)} {html_lib.escape(nro)}</title>
<style>
@page{{size:A4;margin:8mm}}
*{{box-sizing:border-box}}
html,body{{background:#dcdcdc;margin:0;padding:0}}
body{{font-family:Arial,Helvetica,sans-serif;color:#111;font-size:9px}}
.noPrint{{width:210mm;margin:8px auto;text-align:right}}
.noPrint button{{padding:8px 12px;border:1px solid #333;background:#111;color:#fff;border-radius:6px;cursor:pointer}}
.sheet{{width:210mm;height:297mm;margin:0 auto;background:#fff;padding:9mm;position:relative;overflow:hidden}}
.header{{display:grid;grid-template-columns:1fr 19mm 1fr;border:1px solid #777;height:40mm}}
.logoBox{{padding:4mm;border-right:1px solid #777}}
.logo{{max-width:34mm;max-height:16mm;margin-bottom:2mm}}
.centerLetter{{border-right:1px solid #777;text-align:center;position:relative;background:#fff}}
.letterBox{{height:14mm;border-bottom:1px solid #777;display:flex;flex-direction:column;align-items:center;justify-content:center}}
.letter{{font-size:22px;font-weight:800;line-height:1}}
.cod{{font-size:6.5px;margin-top:.5mm}}
.headRight{{padding:4mm 5mm}}
h1{{font-size:16px;margin:0 0 2mm 0;text-align:left;letter-spacing:.02em}}
.small{{font-size:7.5px;color:#222;line-height:1.18}}
.block{{border:1px solid #777;border-top:0;padding:3mm 4mm;height:28mm}}
.two{{display:grid;grid-template-columns:1.12fr .88fr;gap:10mm}}
.line{{margin:.9mm 0}}
.label{{font-weight:bold}}
.itemsWrap{{height:78mm;margin-top:4mm}}
table{{width:100%;border-collapse:collapse;font-size:7.8px}}
th{{background:#eceff1;border:1px solid #777;padding:1.3mm 1mm;text-align:left}}
td{{border:1px solid #aaa;padding:1.05mm 1mm;vertical-align:top;line-height:1.05}}
tr.empty td{{height:5.1mm;color:#fff}}
.c{{text-align:center}}.r{{text-align:right}}
.bottom{{position:absolute;left:9mm;right:9mm;bottom:65mm;display:grid;grid-template-columns:1fr 62mm;gap:5mm;align-items:start}}
.obs{{min-height:28mm;border:1px solid #777;padding:3mm}}
.totals{{border:1px solid #777;padding:3mm;background:#fff}}
.totrow{{display:flex;justify-content:space-between;border-bottom:1px solid #ccc;padding:1.35mm 0;font-size:9px;gap:5mm}}
.totalFinal{{font-size:12px;background:#eee;margin-top:2mm;padding:2mm}}
.fiscalBox{{border:1px solid #777;padding:2mm;margin-bottom:2mm;font-size:8px}}
.auth{{position:absolute;left:9mm;right:9mm;bottom:22mm;display:grid;grid-template-columns:35mm 1fr 62mm;gap:4mm;align-items:end}}
.qr{{height:30mm;border:1px solid #777;display:flex;align-items:center;justify-content:center;font-weight:bold;color:#444;text-align:center}}
.draft{{border:1px solid #111;padding:2mm;margin-top:2mm;text-align:center;font-weight:bold}}
.footer{{font-size:7px;text-align:center;position:absolute;left:9mm;right:9mm;bottom:8mm;color:#555}}
@media print{{html,body{{background:#fff}}.noPrint{{display:none}}.sheet{{width:194mm;height:281mm;margin:0;padding:0;box-shadow:none}}}}
</style>
</head>
<body>
...
<div class="header">
  <div class="logoBox">
    <img src="{html_lib.escape(ARCA_LOGO_URL)}" class="logo" onerror="this.style.display='none'">
    <div><b>{html_lib.escape(ARCA_EMISOR_RAZON_SOCIAL)}</b></div>
    <div class="small">{html_lib.escape(ARCA_EMISOR_DOMICILIO)}<br>{html_lib.escape(ARCA_EMISOR_EMAIL)}<br>{html_lib.escape(ARCA_EMISOR_WEB)}</div>
  </div>
  <div class="centerLetter"><div class="letterBox"><div class="letter">{letter}</div><div class="cod">COD. {int(inv.get('cbte_tipo') or 0):02d}</div></div></div>
  <div class="headRight">
    <h1>{html_lib.escape(title)}</h1>
    <div class="line"><span class="label">Original</span></div>
    <div class="line"><span class="label">Nro:</span> {html_lib.escape(nro)}</div>
    <div class="line"><span class="label">Fecha Emisión:</span> {html_lib.escape(invoice_date)}</div>
    <div class="line"><span class="label">CUIT:</span> {html_lib.escape(ARCA_EMISOR_CUIT)}</div>
    <div class="line"><span class="label">Ing. Brutos:</span> {html_lib.escape(ARCA_EMISOR_IIBB)}</div>
    <div class="line"><span class="label">Inicio Actividad:</span> {html_lib.escape(ARCA_EMISOR_INICIO)}</div>
  </div>
</div>

<div class="block two">
  <div>
    <div class="line"><span class="label">Razón social:</span> {html_lib.escape(str(inv.get('customer_name') or ''))}</div>
    <div class="line"><span class="label">Documento:</span> {html_lib.escape(str(inv.get('customer_doc_type') or ''))} {html_lib.escape(str(inv.get('customer_doc_number') or ''))}</div>
    <div class="line"><span class="label">Condición de IVA:</span> {html_lib.escape(str(inv.get('customer_iva_condition') or ''))}</div>
    <div class="line"><span class="label">Domicilio:</span> {html_lib.escape(str(inv.get('customer_address') or ''))}</div>
  </div>
  <div>
    <div class="line"><span class="label">Concepto:</span> {html_lib.escape(str(inv.get('concept') or ARCA_DEFAULT_CONCEPTO))}</div>
    <div class="line"><span class="label">Canal:</span> {html_lib.escape(str(inv.get('source_channel') or ''))}</div>
    <div class="line"><span class="label">Nº de Operación:</span> {html_lib.escape(str(inv.get('source_order_number') or inv.get('source_order_id') or ''))}</div>
  </div>
</div>

<div class="itemsWrap">
<table>
  <thead><tr><th style="width:9mm">Cant.</th><th style="width:28mm">Código</th><th>Producto / Servicio</th><th style="width:25mm">Precio Unit.</th><th style="width:11mm">Bonif.</th><th style="width:25mm">Subtotal</th>{headers_extra}</tr></thead>
...
<div class="bottom">
  <div class="obs">
    ...
    {html_lib.escape(str(inv.get('notes') or ''))}
    {warning}
  </div>
  <div class="totals">
    {iva_block}
    <div class="totrow totalFinal"><span>Importe Total:</span><b>{arca_money(totals.get('importe_total'))}</b></div>
  </div>
</div>
...
<div class="auth">
  ...
  <div class="small">
    <b>ARCA</b><br>
    Comprobante {'autorizado' if cae else 'pendiente de autorización'}<br>
    Estado ERP: {html_lib.escape(str(status))}
  </div>
  <div class="small">
    <b>CAE Nº:</b> {html_lib.escape(str(cae))}<br>
    <b>Fecha Vto. CAE:</b> {html_lib.escape(str(cae_vto_print))}
  </div>
</div>
<div class="footer">Planeta Casa ERP · Facturación</div>
</div>
</body>
</html>"""
...
def arca_check_endpoint(
    ...
    return {
        ...
        "real_wsfe_enabled": False,
        "nota": "V0 ERP: arma borradores, reporte e impresión A4. CAE real se activa en el siguiente paso con WSFE/certificados.",
    }
